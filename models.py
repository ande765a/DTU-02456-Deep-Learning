...
class SubtitleSegmentation(nn.Module):
	def __init__(self, in_channels, height, width):
		super(SubtitleSegmentation, self).__init__()
		...
		self.has_subtitle_conv = nn.Conv2d(in_channels=512, out_channels=1, kernel_size=2, stride=2)
		self.fc = nn.Linear((height // 2**4) * (width // 2**4), 1)
	...
	def forward(self, X):
		...
		has_subtitles_logits = self.fc(has_subtitles_conv.flatten(start_dim=1))

		mask = torch.sigmoid(mask_logits)
		has_subtitles = torch.sigmoid(has_subtitles_logits)
		
		return mask, has_subtitles
...
class BaselineModel(nn.Module):
	def __init__(self, in_channels, height, width):
		...
		self.fc = nn.Sequential(
			nn.Linear(height * width, 1),
			nn.Sigmoid()
		)

	def forward(self, X):
		mask = self.module(X)
		has_subtitle = self.fc(mask.flatten(start_dim=1))

		return mask, has_subtitle
